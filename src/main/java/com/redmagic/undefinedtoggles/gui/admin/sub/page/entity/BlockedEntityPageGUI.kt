...
class BlockedEntityPageGUI(private val plugin: UndefinedToggles, val list: MutableList<ItemStack>): UndefinedPageMenu("ʙʟᴏᴄᴋᴇᴅ ᴇɴᴛɪᴛʏѕ", MenuSize.LARGE, list) {
    override fun generateInventory(): Inventory = createPageInventory {
        ...
        setItem(50, ItemBuilder(Material.LIME_CONCRETE)
            .setName("<reset><#32e67d>ᴀᴅᴅ ʙʟᴏᴄᴋᴇᴅ ᴇɴᴛɪᴛʏ".translateColor())
            .addLine(" ")
            .addLine("<reset><gray>ᴄʟɪᴄᴋ ᴛᴏ ᴀᴅᴅ ᴀ ʙʟᴏᴄᴋᴇᴅ ᴇɴᴛɪᴛʏ".translateColor()).build())

        addButton(Button(50){

            player.openMenu(ChooseBlockedEntityPageGUI(plugin, Material.SPAWNER.getSpawnEggItems().toMutableList()))
    ...
    override var clickData: ClickData.() -> Unit = {
        ...
        if (item!!.hasItemMeta()){
            ...
            try {
                plugin.configManager.blocks.spawning.blockedMobs.remove(EntityType.valueOf(itemMeta!!.localizedName))
